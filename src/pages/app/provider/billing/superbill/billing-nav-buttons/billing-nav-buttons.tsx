...
const renderTabsData = [
  {
    ...
    path: "/superbill/ready-for-billing",
    lable: "ready-for-billing", // isActive: checkMaster?.dataImport,
    subPath: '/provider/billing/superbill'
  },
  {
    name: "Super Bill",
    path: "/superbill/super-bill",
    lable: "super-bill", // isActive: checkMaster?.labCatalog,
  }
];

function BillingNavButtons() {
  ...
  const RenderButton: React.FC<RenderButtonProps> = ({
    ...
    const toPath = `/provider/billing/superbill/${label}`;
    return (
      <Link to={toPath}>
        <ButtonBase
          className={
...
export default BillingNavButtons;